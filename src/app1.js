    ...
    // npm package "express" // expressモジュールを読み込む https://expressjs.com/ja/4x/api.html
    ...
    const arrMeatData = require('./utils/data'); // data array obj
    
    //
    // import { arrMeatData } from '../public/js/app';
    ...
    // expressアプリを生成する
    const app = express();
    // setup for heroku // ポート3000でサーバを立てる
...
// Setup static directory to serve public フォルダの中身を公開  static.... 静的
    app.use(express.static(publicPath)); // publicPath = ../public

     http://localhost:3000 (Top page) GETメソッドでアクセス
    app.get('', (req, res) => { // req（相手から送られてきた情報）res レスポンス（サーバから送る情報）
...
// Create endpoint API（Meat-stok data obj） を作成
    // http://localhost:3000/api/v1/stockData にアクセス時
    app.get('/api/v1/stockData', (req, res) => {
        // 送るJSONデータ
        res.send( arrMeatData ); // arrMeatData = data array obj from ./utils/data.js
